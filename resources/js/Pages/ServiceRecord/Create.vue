...
const validateDateFrom = () => {
    ...
    const selectedDateFrom = new Date(form.date_from);

    if (selectedDateFrom > currentDate) {
        form.errors.date_from = "Date from cannot be in the future.";
        return false;
    } else {
        form.errors.date_from = null;
        return true;
    }
};

const save = () => {
    // Validate the date_from field
    if (!validateDateFrom()) {
